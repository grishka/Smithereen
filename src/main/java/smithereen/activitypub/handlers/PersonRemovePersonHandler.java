...
public class PersonRemovePersonHandler extends ActivityTypeHandler<ForeignUser, Remove, User>{
	@Override
	public void handle(ActivityHandlerContext context, ForeignUser actor, Remove activity, User object) throws SQLException{
		...
		if(target.equals(actor.getFriendsURL())){
			...
			context.appContext.getNewsfeedController().deleteFriendsFeedEntry(actor, object.id, NewsfeedEntry.Type.ADD_FRIEND);
