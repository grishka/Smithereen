...
public class RenderAttachmentsFunction implements Function{
	...
	private void renderAudioAttachment(AudioAttachment audio, List<String> lines, EvaluationContext evaluationContext, int attachmentIndex, long objectID){
		...
		params.put("durationSeconds", duration==null ? null : duration.getSeconds());
		params.put("url", audio.url);
		PebbleTemplate template=Templates.getTemplate("audio_attachment");
		StringWriter writer=new StringWriter();
		try{
			template.evaluate(writer, params, evaluationContext.getLocale());
		}catch(IOException e){
