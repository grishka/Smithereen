...
void encode_int(int value, int length, std::string& destination);
...
std::string blurHashForPixels(int xComponents, int yComponents, int width, int height, uint8_t *rgb, size_t bytesPerRow) {
	...
	if(acCount > 0) {
		...
		encode_int(quantisedMaximumValue, 1, buffer);
	} else {
		maximumValue = 1;
		encode_int(0, 1, buffer);
	}

	encode_int(encodeDC(dc[0], dc[1], dc[2]), 4, buffer);

	for(int i = 0; i < acCount; i++) {
		encode_int(encodeAC(ac[i * 3 + 0], ac[i * 3 + 1], ac[i * 3 + 2], maximumValue), 2, buffer);
	}
